  ...
  public buildPrompts(
    ...
    systemPrompt +=
      ...
      (chatMode === 'spreadsheet'
        ...
        : `• For multi-row / multi-column output (schedules, comparisons, breakdowns), use plain ` +
          `markdown tables with the COMPUTED numeric values in each cell. Do NOT emit \`\`\`sheet\`\`\` ` +
          `fenced blocks in this mode — those are reserved for spreadsheet mode and will be stripped ` +
          `from the response. If you need to compute multi-row data, calculate the values yourself ` +
          `(or use the single-formula code spans above for one-off values) and present the final ` +
          `numbers in a markdown table.\n` +
          `• NEVER place formula strings ("=B6-B7", "=SUM(C2:C10)", cell refs) inside markdown tables, ` +
          `prose, or anywhere else — they will not be evaluated and the user will see raw formula text. ` +
          `Either compute the value and write the number, or use a single-formula code span.\n`) +
